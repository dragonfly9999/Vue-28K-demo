<template>
  <q-card class="q-pa-md width600">
    ...
    <q-form @submit="() => (createWarn = true)">
          ...
          @click="
            () => {
              const usdt = thousandTool(
                numberTool(balanceRequest.data?.Avb_Balance) * (percent / 100),
                'USDT'
              );
              form.UsdtAmt = usdt;
              price = thousandTool(
      ...
      <div class="flex no-wrap items-center q-my-sm">
        <q-checkbox
          v-model="isPassTwenty"
          dense
          :label="$t('isTwenty.agree')"
        />
        <!-- 免責聲明 -->
        <q-btn flat color="primary" :label="$t('isTwenty.disclaimer')" dense />
        <!-- 使用條款 -->
...
const { run: create, loading } = api.useSell1({
  onSuccess: (res) => {
    ...
    if (token) {
      router.push({
        name: 'trade',
        query: { token, type: 'sell', action: 'trading' },
      });
    }
  },
});
...
// assets
const flag = new URL(`/src/assets/${currency}.png`, import.meta.url).href;
</script>
